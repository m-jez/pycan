...
class CANDriverTests(unittest.TestCase):
    ...
    def testMessageQueues(self):
        ...
        time.sleep(0.5)  # Allow time for the queue to be processed
        self.assertTrue(
            self.driver.total_outbound_count == 2,
            msg="Message not added to outbound queue",
        )
        self.assertTrue(
            self.driver.total_inbound_count == 0,
            msg="Loopback placed a message in the queue",
        )

        self.assertFalse(
            self.driver.send(msg1), msg="Max outbound queue size not honored"
        )
        time.sleep(0.5)  # Allow time for the queue to be processed
        self.assertTrue(
            self.driver.total_outbound_count == 2,
    ...
    def testMessageLoopback(self):
        ...
        self.assertTrue(
            self.driver.total_inbound_count == 2,
            msg="Loopback didn't add message to inbound: %d"
            % self.driver.total_inbound_count,
        )
        ...
        self.assertTrue(
            self.driver.total_inbound_count == 2,
            msg="Loopback still placed the message in the outbound: %d"
            % self.driver.total_inbound_count,
        )
    ...
    def testReceiveHandlers(self):
        ...
        self.assertTrue(
            spec_event1.isSet(), msg="Message 1 specific handler failed"
        )
        self.assertTrue(
            spec_event2.isSet(), msg="Message 2 specific handler failed"
        )
        self.assertTrue(
            spec_event3.isSet(), msg="Message 3 specific handler failed"
        )

        # Check the generic handler
        self.assertTrue(
            gen_event1.isSet(), msg="Message 1 generic handler failed"
        )
        self.assertTrue(
            gen_event2.isSet(), msg="Message 2 generic handler failed"
        )
        self.assertTrue(
            gen_event3.isSet(), msg="Message 3 generic handler failed"
        )
    ...
    def testCyclicPerformance_100ms(self):
        self.driver = BaseDriver(max_in=500, max_out=500, loopback=False)
        self.__performance_test(self.driver, 0.1, 5.0, [1.00, 0.1, 0.175])

    def testCyclicPerformance_10ms(self):
        self.driver = BaseDriver(max_in=500, max_out=500, loopback=False)
        self.__performance_test(self.driver, 0.01, 5.0, [0.5, 0.01, 0.075])
    ...
    def testCyclicAdd(self):
        ...
        self.assertTrue(
            self.driver.add_cyclic_message(msg1, 0.1),
            msg="Unable to add cyclic message",
        )
        self.assertTrue(
            self.driver.add_cyclic_message(msg2, 0.1, "ETC2"),
            msg="Unable to add cyclic message",
        )
        ...
        self.assertTrue(
            self.driver.stop_cyclic_message(msg1.id),
            msg="Unable to stop cyclic message",
        )
        self.assertTrue(
            self.driver.stop_cyclic_message("ETC2"),
            msg="Unable to stop cyclic message",
        )
    ...
    def testCyclicOperation(self):
        ...
        # Add and start some cyclic messages
        self.assertTrue(
            self.driver.add_cyclic_message(msg1, 0.1, "Message 1"),
            msg="Unable to add cyclic message",
        )
        self.assertTrue(
            self.driver.add_cyclic_message(msg2, 0.1, "Message 2"),
            msg="Unable to add cyclic message",
        )
        ...
        self.assertTrue(
            self.driver.update_cyclic_message(msg3, "Message 2"),
            msg="Unable to update cyclic message",
        )
        ...
        self.assertTrue(
            self.driver.stop_cyclic_message("Message 1"),
            msg="Unable to stop cyclic message",
        )
        self.assertTrue(
            self.driver.stop_cyclic_message("Message 2"),
            msg="Unable to stop cyclic message",
        )
    ...
    def __performance_test(self, driver, rate, run_time, tolerances):
        ...
        # Add and start some cyclic messages
        self.assertTrue(
            self.driver.add_cyclic_message(msg1, rate),
            msg="Unable to add cyclic message",
        )
        ...
        self.assertTrue(
            lTarget < avg_t < uTarget,
            msg="Avg time (%1.1f) expected to be between %1.1f and %1.1f"
            % (avg_t, uTarget, lTarget),
        )
        self.assertTrue(
            max_t < uTarget,
            msg="Max time (%1.1f) expected to be less than %1.1f"
            % (max_t, uTarget),
        )
        self.assertTrue(
            min_t > lTarget,
            msg="Min time (%1.1f) expected to be greater than %1.1f"
            % (min_t, lTarget),
        )
